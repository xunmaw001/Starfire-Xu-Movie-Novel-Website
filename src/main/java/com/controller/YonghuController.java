...
public class YonghuController {
    ...
    */
    ...
    public R login(String username, String password, String captcha, HttpServletRequest request) {
        ...
        r.put("huiyuanTypes",yonghu.getHuiyuanTypes());
    ...
    */
    ...
    public R jihuo(Integer  id){
        YonghuEntity yonghuEntity = yonghuService.selectById(id);
        if(yonghuEntity == null || yonghuEntity.getNewMoney() == null)
            return R.error("查不到会员或者会员余额为空");
        if(yonghuEntity.getHuiyuanTypes() == 2)
            return R.error("您已经是会员了");
        Double balance = yonghuEntity.getNewMoney() - 600.0;
        if(balance<0)
            return R.error("您的余额不够600元,不够支付");
        yonghuEntity.setNewMoney(balance);
        yonghuEntity.setHuiyuanTypes(2);
