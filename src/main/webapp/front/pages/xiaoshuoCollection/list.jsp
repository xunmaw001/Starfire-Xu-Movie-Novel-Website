            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">小说收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="xiaoshuoName" id="xiaoshuoName" placeholder="小说名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="xiaoshuoTypes" id="xiaoshuoTypes" lay-filter="xiaoshuoTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in xiaoshuoTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../xiaoshuo/detail.jsp?id='+item.xiaoshuoId)">
                                        <a href="#" class="ng-prsliik">
                                          <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.xiaoshuoPhoto?item.xiaoshuoPhoto.split(',')[0]:''" />
                                          </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../xiaoshuo/detail.jsp?id='+item.xiaoshuoId)"><span>{{item.xiaoshuoName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.xiaoshuoNewMoney"><span style="color: red;">{{item.xiaoshuoNewMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteXiaoshuoCollection(item.id)">删除 {{item.xiaoshuoName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 xiaoshuoCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            xiaoshuoCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteXiaoshuoCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('xiaoshuoCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#xiaoshuoName').val() != null && jquery('#xiaoshuoName').val() != "" && jquery('#xiaoshuoName').val() != "null" ) {
                param['xiaoshuoName'] = jquery('#xiaoshuoName').val();
            }
                     
            var xiaoshuoTypes = $("#xiaoshuoTypes").find("option:selected").val();
            if (xiaoshuoTypes != null && xiaoshuoTypes != "" && xiaoshuoTypes != "null") {
                            param['xiaoshuoTypes'] = xiaoshuoTypes;
                        }
                                                                                
            // 当前表
                                                    // 获取列表数据
            http.request('xiaoshuoCollection/list', 'get', param, function(res) {
...
// 级联表的字典表
       ...
       function xiaoshuoTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=xiaoshuo_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.xiaoshuoTypesList = res.data.list;
               }
           });
       }
